...
test('BACRM evaluation, abstention, fallback, and monitoring remain pure evidence and process checks', () => {
  ...
  const rule = { aiCapabilityId: 'CAP-RANK-001', triggerCondition: 'mandatory input missing or invalid', detectionMethod: 'input validation', requiredResponse: 'Use FBP-RANK', prohibitedResponse: 'Do not rank', fallbackId: 'FB-RANK', testResult: 'Synthetic abstention test passed' };
  ...
  assert.equal(validateAbstentionRule(rule).overallStatus, 'PASS'); assert.equal(validateAbstentionRule({ ...rule, detectionMethod: '' }).overallStatus, 'INCOMPLETE'); assert.equal(validateAbstentionRule({ ...rule, fallbackId: '' }).overallStatus, 'INCOMPLETE'); assert.equal(validateAbstentionRule({ ...rule, testResult: '' }).overallStatus, 'INCOMPLETE');
  ...
  const monitoring = { aiCapabilityId: 'CAP-RANK-001', aiCapabilityVersion: '1.0-synthetic', measure: 'constraint violations', sourceId: 'SYS-AUDIT', frequency: 'per review', threshold: '>=1', observationOwnerId: 'ROLE-OBSERVER', reviewOwnerId: 'ROLE-REVIEWER', immediateResponse: 'Route to review queue', decisionSLA: '4 hours' };
  assert.equal(validateMonitoringTrigger(monitoring).overallStatus, 'PASS'); assert.equal(validateMonitoringTrigger({ ...monitoring, observationOwnerId: '' }).overallStatus, 'INCOMPLETE'); assert.equal(validateMonitoringTrigger({ ...monitoring, reviewOwnerId: '' }).overallStatus, 'INCOMPLETE'); assert.equal(validateMonitoringTrigger({ ...monitoring, immediateResponse: '' }).overallStatus, 'INCOMPLETE'); assert.equal(validateMonitoringTrigger({ ...monitoring, decisionSLA: '' }).overallStatus, 'INCOMPLETE');
});
...
test('BACRM suspension preserves history and recovery requires three independent gates without reactivation', () => {
  ...
  assert.equal(suspension.historicalOutputsPreserved, false); assert.equal(suspensionResult.overallStatus, 'INCOMPLETE');
  assert.equal(validateAISuspension({ ...suspension, historicalOutputsPreserved: undefined }, { id: 'DEC-SUS', decisionOwnerId: 'PAR-HARBOR' }, authority, fallback, '2027-02-02T00:00:00Z').overallStatus, 'INCOMPLETE');
  assert.equal(validateAISuspension({ ...suspension, historicalOutputsPreserved: true }, { id: 'DEC-SUS', decisionOwnerId: 'PAR-HARBOR' }, authority, fallback, '2027-02-02T00:00:00Z').overallStatus, 'PASS');
...
test('BACRM bounded release is human-authorized and lifecycle requires later explicit release after suspension', () => {
  const releaseInput = { capability: { id: 'CAP-RANK-001', version: '1.0-synthetic', purpose: 'Rank eligible options' }, baseline, fallback, inputBoundaries: [input], outputBoundaries: [output], authorityCrosswalks: [crosswalk], evaluations: [{ aiCapabilityId: 'CAP-RANK-001', version: '1.0-synthetic', evaluationQuestion: 'Recall', metric: 'top-3 recall', acceptanceRule: '>=95', observedResult: 97 }], abstentionRules: [{ aiCapabilityId: 'CAP-RANK-001', triggerCondition: 'missing input', detectionMethod: 'input validation', requiredResponse: 'fallback', prohibitedResponse: 'rank', fallbackId: 'FB-RANK', testResult: 'Synthetic test passed' }], monitoringTriggers: [{ aiCapabilityId: 'CAP-RANK-001', aiCapabilityVersion: '1.0-synthetic', measure: 'constraint violations', sourceId: 'SYS-AUDIT', frequency: 'per review', threshold: '>=1', observationOwnerId: 'ROLE-OBSERVER', reviewOwnerId: 'ROLE-REVIEWER', immediateResponse: 'Route to review queue', decisionSLA: '4 hours' }], releaseCriteria: [{ aiCapabilityId: 'CAP-RANK-001', criterionType: 'baseline', requiredState: 'PASS', observedState: 'PASS' }], authorityEnvelope: authority, permission, commitment: { id: 'COM-RANK', state: 'ACCEPTED' }, asOfTime: '2027-01-02T00:00:00Z' };
  const release = evaluateBoundedRelease(releaseInput);
  assert.equal(release.status, 'READY_FOR_BOUNDED_RELEASE_DECISION'); assert.equal(release.readyForReleaseDecision, true); assert.equal(release.releaseAllowed, false); assert.equal(release.releaseAuthorizationStatus, 'NOT_DECIDED'); assert.equal(release.autonomousActivation, false);
  ...
  assert.equal(evaluateBoundedRelease({ ...releaseInput, accountableDecision: { aiCapabilityId: 'CAP-RANK-001' } }).releaseAuthorizationStatus, 'INCOMPLETE');
  assert.equal(evaluateBoundedRelease({ ...releaseInput, accountableDecision: { aiCapabilityId: 'CAP-RANK-001', version: '1.0-synthetic', disposition: 'RELEASED_BOUNDED', decisionOwnerId: 'PAR-HARBOR', authorityId: 'AE-RANK', effectiveTime: '2027-01-02T00:00:00Z' } }).releaseAllowed, true);
  assert.equal(evaluateBoundedRelease({ ...releaseInput, releaseCriteria: [{ aiCapabilityId: 'CAP-RANK-001', criterionType: 'baseline', requiredState: 'PASS', observedState: 'FAIL' }], accountableDecision: { aiCapabilityId: 'CAP-RANK-001', version: '1.0-synthetic', disposition: 'RELEASED_BOUNDED', decisionOwnerId: 'PAR-HARBOR', authorityId: 'AE-RANK', effectiveTime: '2027-01-02T00:00:00Z' } }).releaseAllowed, false);
  assert.equal(evaluateBoundedRelease({ ...releaseInput, abstentionRules: [{ aiCapabilityId: 'CAP-RANK-001' }] }).status, 'INCOMPLETE'); assert.equal(evaluateBoundedRelease({ ...releaseInput, monitoringTriggers: [{ aiCapabilityId: 'CAP-RANK-001' }] }).status, 'INCOMPLETE');
