...
export function calculateScenarioEconomicIncrement({ workspace = {}, selectedCaseId = '', calculatedCases = [] } = {}) { const comparison = scenarioComparison(workspace, selectedCaseId); if (comparison.comparatorResolutionStatus !== 'RESOLVED') return { status: 'INCOMPLETE', attribution: 'UNRESOLVED', issues: comparison.issues }; const selected = recordArray(calculatedCases).find(item => item.caseId === selectedCaseId); const comparator = recordArray(calculatedCases).find(item => item.caseId === comparison.comparatorCase.id); if (!selected || !comparator) return { status: 'INCOMPLETE', attribution: comparison.attribution, issues: ['Calculated comparator case is missing.'] }; return { status: 'PASS', attribution: comparison.attribution, collectiveNPV: selected.collectiveNPV - comparator.collectiveNPV, collectiveROI: selected.collectiveROI - comparator.collectiveROI, benefitCostRatio: selected.benefitCostRatio / comparator.benefitCostRatio, issues: [] }; }
