...
The public comparator-aware calculation receives calculated case outputs carrying their compatible pre-PV cash-flow schedules and derives incremental NPV, ROI, and BCR inside `calculateScenarioEconomicIncrement()` from a period-index-aligned selected-minus-comparator schedule. The public path reproduces the C2-C1 and C3-C2 targets. ROI is never calculated by subtracting case-level ROIs, and BCR is never calculated by dividing case-level BCRs. Member allocation remains conditional, and all results remain synthetic process-test evidence only.
