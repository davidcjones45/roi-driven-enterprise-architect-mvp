...
Incremental NPV, ROI, and BCR are recomputed from a period-index-aligned selected-minus-comparator cash-flow schedule. ROI is never calculated by subtracting case-level ROIs, and BCR is never calculated by dividing case-level BCRs. Member allocation remains conditional, and all results remain synthetic process-test evidence only.
